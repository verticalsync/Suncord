...
window.VencordNative = {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/verticalsync/Suncord" }),
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("SuncordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("SuncordSettings", JSON.stringify(s)),
